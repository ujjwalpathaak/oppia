...
export class QuestionsListComponent implements OnInit, OnDestroy {
  ...
  changeLinkedSkillDifficulty(): void {
    ...
    if (this.newQuestionSkillIds.length === 1) {
      ...
      this.linkedSkillsWithDifficulty.forEach(linkedSkillWithDifficulty => {
        if (
          !this.newQuestionSkillIds.includes(linkedSkillWithDifficulty.getId())
        ) {
    ...
    this.skillBackendApiService.fetchMultiSkillsAsync(skillIds).then(
      skills => {
        skills.forEach(skill => {
          this.misconceptionsBySkill[skill.getId()] = skill.getMisconceptions();
        });
      },
      error => {
  ...
  editQuestion(
    ...
    this.editableQuestionBackendApiService
      .fetchQuestionAsync(questionSummaryForOneSkill.getQuestionId())
      .then(
        response => {
  ...
  openQuestionEditor(): void {
    ...
    this.editorIsOpen = true;
    this.topicEditorStateService.toggleTopicSkillEditor(true);
    this.imageLocalStorageService.flushStoredImagesData();
  ...
  removeQuestionSkillLinkAsync(
    ...
    this.editableQuestionBackendApiService
      .editQuestionSkillLinksAsync(questionId, [
        {
          id: skillId,
          task: 'remove',
  ...
  removeSkill(skillId: string): void {
    ...
    this.associatedSkillSummaries = this.associatedSkillSummaries.filter(
      summary => {
  ...
  addSkill(): void {
    let skillsInSameTopicCount = this.groupedSkillSummaries.current.length;
    ...
    modalRef.result.then(
      summary => {
        for (let idx in this.associatedSkillSummaries) {
          if (this.associatedSkillSummaries[idx].getId() === summary.id) {
            this.alertsService.addInfoMessage(
  ...
  getQuestionIndex(index: number): number {
    return (
      this.questionsListService.getCurrentPageNumber() *
        AppConstants.NUM_QUESTIONS_PER_PAGE +
      index +
      1
    );
  }
    ...
    this.ngbModal
      ...
      .result.then(
        () => {
          ...
          this.editorIsOpen = false;
          this.topicEditorStateService.toggleTopicSkillEditor(false);
  ...
  _initTab(resetHistoryAndFetch: boolean): void {
    ...
    if (this.selectedSkillId) {
      this.skillBackendApiService
        .fetchSkillAsync(this.selectedSkillId)
        .then(responseObject => {
  ...
  ngOnInit(): void {
    ...
    this.directiveSubscriptions.add(
      this.topicEditorStateService.onSkillEditorOpened.subscribe()
    );
